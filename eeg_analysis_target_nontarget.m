...
for i = 1:8
    ...
    subplot(2,4,i)
    pop_erpimage(EEG,0, [i],[[]],['Komponenta ', int2str(i)],10,1,{},[],'' ,'yerplabel','','erp','on','cbar','on','topo', { mean(EEG.icawinv(:,[i]),2) EEG.chanlocs EEG.chaninfo } );
end
...
for j = 9:16
    subplot(2,4,j-8)
    pop_newtimef( EEG, 0, j, [0  1998], [3         0.8] , 'topovec', EEG.icawinv(:,j), 'elocs', EEG.chanlocs, 'chaninfo', EEG.chaninfo, 'caption',['IC 9. - 16. komponenta '], 'baseline',[0], 'plotphase', 'off', 'padratio', 1);
